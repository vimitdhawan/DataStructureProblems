...
class Flatttening_A_LinkedList
{
    ...
    public  static void main(String args[])
    {
        ...
        while(t>0)
        {
            ...
            //list.printList();
            Flatttening_A_LinkedListSolution g = new Flatttening_A_LinkedListSolution();
...
class Flatttening_A_LinkedListSolution
{
    Node flatten(Node root)
    {
        if(root == null || root.next==null) return root;
        root.next = flatten(root.next);
        Node mergeNode = merge(root, root.next);
        return mergeNode;
    }
    ...
    Node merge(Node firstHead, Node secondHead){
        ...
        if(firstHead.data<secondHead.data){
            result = firstHead;
            result.bottom = merge(firstHead.bottom, secondHead);
        }else{
            result = secondHead;
            result.bottom = merge(firstHead, secondHead.bottom);
        }
        result.next = null;
        return result;
